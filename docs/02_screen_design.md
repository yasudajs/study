...
- 段選択: 1〜9段（複数選択）
- 出題方式: 順番／ランダム
- 出題: 「a × b = ?」にテンキー入力、?へリアルタイム表示
- 結果: 正答率・履歴、再挑戦／設定変更／ポータルへ
...
設定画面:
- 演算選択: ＋/−/×/÷（複数）
- 難易度: かんたん／むずかしい（割り算は余りあり）
- 範囲: 1桁（0〜9）／上限値指定

出題画面:
- 問題表示（商・余りの場合は入力切替ボタン: 「こたえを入力中／あまりを入力」）
- テンキー（0〜9、クリア、符号、答える）

結果画面:
- 正答数・正答率・履歴
- ボタンは縦並び右寄せ、サイズ統一（min-width: 200px）

---

## 共通UI仕様
- ボタン: 最小44×44px、ホバー/アクティブ時のフィードバック
- フォント: 読みやすい太字、適切なサイズ
- 進捗表示: 問題番号とプログレスバー

## PWA・レスポンシブ
- ブレークポイント: ~599 / 600–899 / 900+
- モバイルで1列、タブレット以上でグリッド表示# 画面設計書
...
| 要素 | 説明 | 仕様 |
|------|------|------|
| タイトル | 「かける数を選択してください」 | 24～32px |
| チェックボックス | 1～9の段 | 3列配置 |
| リセットボタン | すべての選択を解除 | 幅40%、高さ50px |
| 進むボタン | 選択完了 | 幅40%、高さ50px |

### 検証
- **エラー**: 1つも段が選択されていない場合、進むボタンはグレーアウト（クリック不可）
- **成功**: 1つ以上選択 → 進むボタンが有効化

### 動作
- **進むボタンをクリック** → **出題方式選択画面**へ遷移（選択情報を保持）
