...
| 3 | ユーザー回答を待機 | **クライアント（テンキー入力）** | テンキーでの回答を入力待機 |
| 4 | テンキー入力時の表示 | **クライアント（リアルタイム更新）** | 問題文の「?」部分に入力値をリアルタイム表示 |
| 5 | 「次へ」クリック | **クライアント（JavaScript）** | 回答を記録 |
| 6 | 正誤判定 | **クライアント（JavaScript）** | ユーザーの回答と正答を比較 |
| 7 | 次問へ遷移 | **クライアント（JavaScript）** | 残り問題があれば次問へ、なければ結果画面へ |

### 4.4 エラーハンドリング
| エラー | メッセージ | 動作 | 実施場所 |
|--------|-----------|------|--------|
| 空入力 | 「答えを入力してください」 | 次へボタン無効化 | **クライアント** |
| 非数値入力 | テンキーのみで入力可能 | テンキー選択のみで入力を制限 | **クライアント** |
...
### 11.1 ポータル機能
- アプリ一覧は `app/portal/logic.py` の `get_available_apps()` を参照
- 画面は `app/templates/portal/index.html` を使用

### 11.2 四則演算機能
- 設定画面: 演算（＋/−/×/÷）、難易度（かんたん/むずかしい）、範囲（1桁/上限値）
- 割り算むずかしいの入力切替: 「こたえを入力中／あまりを入力」
- 入力切替ボタンはテンキーと同幅・右寄せ・改行なし
- 結果画面のボタンは縦並び右寄せ・サイズ統一（min-width: 200px）

### 11.3 クライアント中心の設計再確認
- 問題生成・採点・正答率計算はクライアント側で実施
- サーバーはセッション作成／結果保存のみ（将来拡張）
